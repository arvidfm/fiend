...
int los_buffer_check(int x, int y)
{
	...
	if(x + tile_pos_x<0)return 1;
	if(y + tile_pos_y<0)return 1;
	if(x + tile_pos_x>=map->w)return 1;
	if(y + tile_pos_y>=map->h)return 1;
...
void draw_los_buffer(BITMAP *dest, int xpos, int ypos)
{
	...
	for(i=0;i< (virt->w/TILE_SIZE+1) ;i++)
		for(j=0;j< (virt->h/TILE_SIZE+1) ;j++)
		{
			...
			else if(los_buffer[i][j]==0 && !tile_is_wall_solid(i+xpos/32, j+ypos/32))
			{
				
				//--Border--//

				//border right
				if(los_buffer_check2(i-1,j) && !los_buffer_check2(i,j-1) && !los_buffer_check2(i,j+1) )
				{
					draw_lightsprite(dest,los_border[0][0],x1+(i*TILE_SIZE), y1+(j*TILE_SIZE));
				}
				
				//border down
				if(los_buffer_check2(i,j-1) && !los_buffer_check2(i-1,j) && !los_buffer_check2(i+1,j) )
				{
				...
				//right
				if(los_buffer_check2(i-1,j) && los_buffer_check2(i,j+1) && !los_buffer_check2(i+1,j) && !los_buffer_check2(i,j-1))
				{
				...
				//right
				if(los_buffer_check2(i-1,j+1) && !los_buffer_check2(i-1,j) && !los_buffer_check2(i,j+1))
				{
				...
				//right
				if(los_buffer_check2(i,j-1) && los_buffer_check2(i+1,j) && los_buffer_check2(i,j+1))
				{
...
void update_los_buffer(int xpos, int ypos)
{
	...
	for(i=0;i<18;i++)
		...
		if(!tile_is_wall_solid(i+xpos/32, j+ypos/32) )
		{
			...
			if(j>0)
			{
				if(los_buffer[i][j-1]==1 || tile_is_wall_solid(i+xpos/32, (j-1)+ypos/32))
					c1=1;
			}
			else
			{
			...
			if(i<17)
			{
				if(los_buffer[i+1][j]==1 || tile_is_wall_solid((i+1)+xpos/32, (j)+ypos/32))
					c2=1;
			}
			else
			{
		...
		for(i=0;i<18;i++)
			for(j=0;j<18;j++)
			{
				...
				{
					//--Border--//
					if(los_buffer_check(i-1,j) && los_buffer_check(i,j-1) && los_buffer_check(i,j+1) && !los_buffer_check(i+1,j))
					{
						temp_los_buffer[i][j]=0;
					}
					else if(los_buffer_check(i,j-1) && los_buffer_check(i-1,j) && los_buffer_check(i+1,j) && !los_buffer_check(i,j+1))
					{
						temp_los_buffer[i][j]=0;
					}
					else if(los_buffer_check(i+1,j) && los_buffer_check(i,j-1) && los_buffer_check(i,j+1) && !los_buffer_check(i-1,j))
					{
						temp_los_buffer[i][j]=0;
					}
					else if(los_buffer_check(i,j+1) && los_buffer_check(i-1,j) && los_buffer_check(i+1,j) &&  !los_buffer_check(i,j-1))
					{
					 ...
					 //--Inner Corner--//
					else if(los_buffer_check(i-1,j) && los_buffer_check(i,j+1) && !los_buffer_check(i+1,j) && !los_buffer_check(i,j-1))
					{
						temp_los_buffer[i][j]=0;
					}
					else if(los_buffer_check(i-1,j) && los_buffer_check(i,j-1) && !los_buffer_check(i+1,j) && !los_buffer_check(i,j+1))
		...
		//an extra check to remove "bugs"
		//improve the line of dight func and this can be taken away.
		/*for(k=0;k<2;k++)
		for(i=0;i<18;i++)
		for(j=0;j<18;j++)
		if(!tile_is_wall_solid(i+xpos/32, j+ypos/32) )
